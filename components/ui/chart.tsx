...
function ChartContainer({
  ...
  React.useEffect(() => {
    ...
    const checkSize = (entries?: ResizeObserverEntry[]) => {
      const target = entries ? entries[0].target : containerRef.current;
      if (target) {
    ...
    // ResizeObserver is more efficient than intervals
    if (typeof ResizeObserver !== "undefined" && containerRef.current) {
      ...
      // Fallback to interval for older environments
      const interval = setInterval(() => {
        if (checkSize()) clearInterval(interval);
      }, 100);
  ...
  return (
    <ChartContext.Provider value={{ config }}>
      ...
      >
        ...
        {isMounted && hasSize ? (
          <div className="absolute inset-0 w-full h-full" style={{ minWidth: 0, minHeight: 0 }}>
          ...
          <div className="flex items-center justify-center w-full h-full text-slate-500 font-bold uppercase tracking-widest text-[10px] animate-pulse">
